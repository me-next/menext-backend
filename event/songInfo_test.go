...
func TestSongInfo(t *testing.T) {
	...
	np.ChangeSong(queue.SongUID("1"))

	getRaw := func(np *event.NowPlaying) map[string]interface{} {
		raw := np.Data()
	...
	// get the start time
	data := getRaw(np)
	startT := toUnix(data[event.KSongStartTimeMs])

	// wait a little
	time.Sleep(100 * time.Millisecond)

	data = getRaw(np)
	unchangedStartT := toUnix(data[event.KSongStartTimeMs])
	currentT := toUnix(data[event.KCurrentTimeMs])
